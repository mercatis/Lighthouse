...
public class DeploymentContainerDropTargetAdapter implements DropTargetAdapter<DeploymentContainer> {
	 ...
	 */
	public boolean handleDrop(int operation, DeploymentContainer target, Object[] elements, Shell shell) {
		LighthouseDomain targetDomain = CommonBaseActivator.getPlugin().getDomainService().getLighthouseDomainByEntity(target);
		for (Object element : elements) {
			if (element instanceof SoftwareComponent) {
				SoftwareComponent component = (SoftwareComponent) element;
				NewDeploymentWizard wizard = new NewDeploymentWizard();
				wizard.setLighthouseDomain(targetDomain);
	 ...
	 */
	public boolean validateDrop(int operation, DeploymentContainer target, Object[] elements) {
