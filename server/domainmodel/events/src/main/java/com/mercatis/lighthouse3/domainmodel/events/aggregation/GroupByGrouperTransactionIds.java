...
/**
 * This iterator combines events to groups, by using the transaction ids
 * property, in every iteration step. The group of events, which will be divided
 * into subgroups, for each iteration step is provided by an
 * <code>IntervalGrouper</code>.
 */
public class GroupByGrouperTransactionIds implements GroupByGrouper {
	...
	private void addToGroupDistinctly(ArrayList<Group<Event>> list, Event e) {
		...
		for (String s : e.getTransactionIds()) {
			groupName += s + " ";
		}
		...
		if (!found) {
			...
			newList.add(e);
			list.add(new Group<Event>(newList, groupName));
		}
	}
