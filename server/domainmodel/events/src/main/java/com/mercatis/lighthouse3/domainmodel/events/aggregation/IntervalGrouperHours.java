 ...
 * 'hours', in every iteration step. The list of events is retrieved by an
 * iterator over <code>Event</code>s.
 */
public class IntervalGrouperHours implements IntervalGrouper {
	...
	/**
	 * Saves the day of the last group.
	 */
	private int lastDay = Integer.MAX_VALUE;
	 ...
	 */
	private int lastHour = Integer.MAX_VALUE;

	/**
	 * Used for retrieving a group identification from a date.
	 */
	private SimpleDateFormat dateFormat = new SimpleDateFormat("dd.MM.yyyy, HH:00");
	...
	public Group<Event> next() {
		...
		while (iterator.hasNext()) {
			...
			if (thisHour == -1)
				if (isBeforeLastDate(year, month, day, hour)) {
					thisYear = year;
					thisMonth = month;
					thisDay = day;
					thisHour = hour;
				}
			if (year == thisYear && month == thisMonth && day == thisDay && hour == thisHour) {
				events.add(e);
	 ...
	 * @param hour
	 *            the hour of the date to be checked
	 * @return true if it is older, false otherwise
	 */
	private boolean isBeforeLastDate(int year, int month, int day, int hour) {
		return (year < lastYear) || (year == lastYear && month < lastMonth)
				|| (year == lastYear && month == lastMonth && day < lastDay)
				|| (year == lastYear && month == lastMonth && day == lastDay && hour < lastHour);
	}
