...
/**
 * Iterator that delivers summed value of a property of <code>Event</code> in a
 * group of events in each iteration step. This only works for a UDF.
 */
public class AggregatorSum implements Aggregator {
	...
	/**
	 * The aggregation target to compute the summed value from.
	 */
	...
	public AggregationIntervalResult next() {
		...
		if (target.getType().equals(Type.EVENTS) || target.getType().equals(Type.LEVEL)
				|| target.getType().equals(Type.MACHINE_OF_ORIGIN) || target.getType().equals(Type.DATE)) {
			throw new AggregationException("Not a summable type.", null);
	...
	/**
	 * Returns the summed value of a certain UDF in a group of events.
	 * 
	 * @param group
	 *            the group of events
	 * @param udfName
