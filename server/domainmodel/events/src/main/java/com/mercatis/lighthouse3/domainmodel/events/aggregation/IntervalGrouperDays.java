 ...
 * 'days', in every iteration step. The list of events is retrieved by an
 * iterator over <code>Event</code>s.
 */
public class IntervalGrouperDays implements IntervalGrouper {
	...
	public Group<Event> next() {
		...
		while (iterator.hasNext()) {
			...
			if (thisDay == -1)
				if (isBeforeLastDate(year, month, day)) {
					thisYear = year;
					thisMonth = month;
					thisDay = day;
				}
	 ...
	 */
	private boolean isBeforeLastDate(int year, int month, int day) {
