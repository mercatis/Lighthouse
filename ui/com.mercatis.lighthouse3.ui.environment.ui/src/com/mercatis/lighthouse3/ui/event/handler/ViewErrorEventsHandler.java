...
public class ViewErrorEventsHandler extends AbstractStructuredSelectionHandler {
	...
	protected void execute(Object element) throws ExecutionException {
		...
		Event template = EventBuilder.template().setDateOfOccurrence(Ranger.interval(new Date(System.currentTimeMillis() - 6 * 60 * 60 * 1000), null)).done();
		...
		s.add("ERROR");
		s.add("FATAL");
		template.setLevel(Ranger.enumeration(s));
