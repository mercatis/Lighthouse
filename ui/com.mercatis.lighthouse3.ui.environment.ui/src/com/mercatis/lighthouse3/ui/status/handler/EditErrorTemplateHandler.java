...
public class EditErrorTemplateHandler extends AbstractStructuredSelectionHandler {
	...
	protected void execute(Object element) throws ExecutionException {
		if (element instanceof StatusEditingObject) {
			...
			TemplateEditor wizard = new TemplateEditor(lighthouseDomain, status.getErrorTemplate());
			WizardDialog dialog = new WizardDialog(Display.getCurrent().getActiveShell(), wizard);
			dialog.create();
			dialog.getShell().setText("Template editor");
			dialog.setTitle("Edit Error template on " + LabelConverter.getLabel(status));
			dialog.setMessage("An event matching this template will result in an erroneous status.");
