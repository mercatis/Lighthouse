...
/**
 * Iterator that delivers the highest value of a property of <code>Event</code>
 * in a group of events in each iteration step.
 */
public class AggregatorMaximum implements Aggregator {
	...
	/**
	 * The aggregation target to compute the highest value from.
	 */
	...
	/**
	 * Returns the highest value of a certain UDF in a group of events.
	 * 
	 * @param group
	 *            the group of events
	 * @param udfName
	...
	private Object addByUDF(Group<Event> group, String udfName) {
		int integerResult = Integer.MIN_VALUE;
		double doubleResult = Double.MIN_VALUE;
		float floatResult = Float.MIN_VALUE;
		...
		if (doubleResult != Double.MIN_VALUE) {
			return doubleResult;
		} else if (floatResult != Float.MIN_VALUE) {
			return floatResult;
		} else if (longResult != Long.MIN_VALUE) {
			return longResult;
		} else {
	...
	/**
	 * Returns the most actual date of occurrence in a group of events.
	 * 
	 * @param group
	 *            the group of events
	 * @return the most actual date
	 */
