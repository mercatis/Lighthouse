...
/**
 * This iterator combines events to groups, by using the level property, in
 ...
 */
public class GroupByGrouperLevel implements GroupByGrouper {
	...
	private void addToGroupDistinctly(ArrayList<Group<Event>> list, Event e) {
		...
		for (Group<Event> group : list)
			if (group.getIdentifier().equals(e.getLevel())) {
				group.getList().add(e);
				found = true;
			}
		...
		if (!found) {
			...
			newList.add(e);
			list.add(new Group<Event>(newList, e.getLevel()));
		}
	}
