...
/**
 * This iterator combines events to groups, by using the context property, in
 * every iteration step. The group of events, which will be divided into
 * subgroups, for each iteration step is provided by an
 * <code>IntervalGrouper</code>.
 */
public class GroupByGrouperContext implements GroupByGrouper {
	...
	private void addToGroupDistinctly(ArrayList<Group<Event>> list, Event e) {
		...
		for (Group<Event> group : list)
			if (group.getIdentifier().equals(
					e.getContext().getLocation() + ", " + e.getContext().getDeployedComponent().getCode())) {
				group.getList().add(e);
				found = true;
			}
		...
		if (!found) {
			...
			newList.add(e);
			list.add(new Group<Event>(newList, e.getContext().getLocation() + ", "
					+ e.getContext().getDeployedComponent().getCode()));
		}
	}
